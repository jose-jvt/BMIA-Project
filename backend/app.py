...
from classify import model_inference
...
def export_to_cm():
    ...
    if file_path.endswith(".nii") or file_path.endswith(".nii.gz"):
        ...
        if not bval_name or not bvec_name:
            return (
                jsonify(
                    {
                        "error": "When processing NIFTI files .bval and .bvec are required"
                    }
                ),
                400,
            )
    ...
    pipeline = DSIProcessor(
        ...
        output_prefix=os.path.join(
            ...
            os.path.basename(file_path).split(".")[0] + "_CM",
        ),
        atlas=atlas,
    ...
    return (
        jsonify(
            {
                "message": f"Connectivity matrix exported at {os.path.dirname(file_path)}",
            }
        ),
        200,
    )
...
def execute_model():
    mat_connectivity = request.json["matrix_path"]
    model_path = request.json["model_path"]
    inferencia = model_inference(mat_connectivity, model_path)
    return jsonify({"message": f"Inference result: {inferencia}"}), 200
